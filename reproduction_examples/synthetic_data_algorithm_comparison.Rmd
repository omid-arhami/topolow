...
This vignette demonstrates how to perform a rigorous comparison between Topolow and RACMACS algorithms for antigenic cartography. We conduct k-fold cross-validation and evaluate performance using multiple metrics including mean absolute error (MAE), correlation, and coverage.
...
# Parameters
#random_seed <- 6711  # For reproducibility 

# Setup optimization parameters
opt_params <- list(
  n_folds = 20,
  max_topolow_iter = 2000, # Cutoff of one optimization
  ...
  final_results = read.csv("final_parameters_80points.csv"),  # From parameter fitting
  use_slurm = FALSE
)
...
for (dir in c(matrix_dir, results_dir)) {
  if (!dir.exists(dir)) {
...
for (ndim in results$metadata$dimension) {
  ...
  for (matrix_type in matrix_variants) {
    ...
    best_params <- opt_params$final_results %>%
    ...
    best_params <- list(
      ...
      k_decay = exp(best_params$log_k_decay),
      cqq = exp(best_params$log_cqq)
    )
    ...
    for(i in seq_len(opt_params$n_folds)) {
      ...
      if (opt_params$use_slurm) {
        ...
        cat(sprintf("Scenario %d out of %d \n", cc, length(results$metadata$dimension)*length(matrix_variants)))
...
p <- ggplot(all_mape_results, aes(x = Missing, y = MAPE, fill = Method)) +
  ...
  labs(
    ...
    y = "Mean Absolute Percentage Error (±SD)"
  )

# Save plot
ggsave("Method_comparison_MAPE_plot.pdf", plot = p, width = 172, height = 0.5*172, units = "mm", dpi = 300)
```
...
ggsave("Method_comparison_MAE_plot.pdf", plot = p, width = 7, height = 4.5, dpi = 300)
...
```{r}
# all_cor_results <- rbind(topolow_cor_stats, racmacs_cor_stats)
# 
# all_cor_results$Variant <- factor(all_cor_results$Variant, levels = c("Original", "+Noise", "+Noise+Bias"))
# 
# p <- ggplot(all_cor_results, aes(x = Missing, y = MAE, fill = Method)) +
#   geom_bar(stat = "identity", position = position_dodge()) +
#   geom_errorbar(aes(ymin = MAE - SD, ymax = MAE + SD),
#                 position = position_dodge(width = 0.9), width = 0.25) +
#   facet_grid(Dimension ~ Variant) +
#   scale_fill_manual(values = c("Topolow" = "#4dbedf", "MDS" = "#DC3912")) +
#   theme_minimal() +
#   theme(
...
#     x = "Missing Data Level",
#     y = "Pearson r (±SD)",
#     title = "Comparison of Topolow and MDS Performance",
#     subtitle = "Across dimensions, missing data levels, and data variants"
#   )
# # Save the plot
# ggsave("Method_comparison_cor_plot.png", plot = p, width = 10, height = 8, dpi = 300)
# 
# print(p)
```
...
for (ndim in results$metadata$dimension) {
  for (variant in noise_variants) {
    topo <- error_reduction_analysis(ndim, variant, topolow_errors_df, results)
    mds <- error_reduction_analysis(ndim, variant, racmacs_2D_errors_df, results)
...
coords_dir <- file.path("results", "coordinates")
plots_dir <- file.path("results", "plots")
dir.create(coords_dir, recursive = TRUE, showWarnings = FALSE)
...
for(ndim in results$metadata$dimension) {
  ...
  original_coords <- results$coordinates[[which(results$metadata$dimension == ndim)]]
...
```{r}
ndim = results$metadata$dimension[3]
...
dim_data <- results$matrices[[which(results$metadata$dimension == ndim)]][[14]]
...
amc_files <- c(
  file.path("hyperparam_results",
            sprintf("synthetic_dim%d_%s_hyperparam_results.csv", 
                   ndim, matrix_type)),
  file.path("hyperparam_results",
            sprintf("synthetic_MC2_dim%d_%s_hyperparam_results.csv", 
                   ndim, matrix_type)),
  file.path("hyperparam_results",
            sprintf("synthetic_MC7_dim%d_%s_hyperparam_results.csv", 
                   ndim, matrix_type))
)
...
optimal_params <- list(
  ...
  k_decay = exp(as.numeric(optimal_params$log_k_decay)),
  cqq = exp(as.numeric(optimal_params$log_cqq))
)
