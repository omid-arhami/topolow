...
We conduct 20-fold cross-validation and evaluate performance using multiple metrics including mean absolute error (MAE), correlation, and coverage.
...
opt_setting <- list(
  n_folds = 20,
  max_topolow_iter = 4000,
  topolow_opt_rounds = 1,
  racmacs_opt_rounds = 1000
)
...
for (dataset_name in names(datasets)) {
  ...
  topo_result <- tryCatch({
    topolow_full(
      ...
      ndim = params$N,
      max_iter = opt_setting$max_topolow_iter,
...
if (any(sapply(datasets, function(x) x$has_clusters))) {
  h3n2_metadata <- h3n2_results$long %>%
  ...
  for(i in seq_len(opt_setting$n_folds)) {
    ...
    coverage_df <- rbind(
      ...
      data.table(
        ...
        Validation_Coverage = topo_errors$coverage,
        Algorithm = "Topolow",
    ...
    cor_df <- rbind(
      cor_df,
      data.table(
        Validation_Cor = topo_errors$OutSampleCor,
        Algorithm = "Topolow",
        Dataset = dataset_name,
        Fold = i
      )
...
write.csv(cor_df, file.path(output_dir, "cor_df_loop.csv") )
write.csv(coverage_df, file.path(output_dir, "coverage_df_loop.csv") )
```
...
cor_df <- read.csv(file.path(output_dir, "cor_df_loop.csv"))
coverage_df <- read.csv(file.path(output_dir, "coverage_df_loop.csv"))
```
...
coverage_summary <- coverage_df %>%
  group_by(Dataset, Algorithm) %>%
  summarize(
    Mean_Coverage = mean(Validation_Coverage),
    SD_Coverage = sd(Validation_Coverage),
    .groups = 'drop'
  ) %>%
  mutate(
    Coverage = sprintf("%.1f%% ± %.1f%%", 
                      Mean_Coverage * 100, 
                      SD_Coverage * 100)
  ) %>%
  select(Dataset, Algorithm, Coverage) %>%
  tidyr::pivot_wider(names_from = Algorithm,
