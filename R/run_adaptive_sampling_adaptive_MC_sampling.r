run_adaptive_sampling <- function(initial_samples_file,
  ...
  if (use_slurm) {
    # SLURM: write distance matrix and submit jobs
    ...
    for (i in seq_len(num_parallel_jobs)) {
      ...
      args <- c(temp_f, dist_file, as.character(mapping_max_iter), as.character(relative_epsilon),
                "1", scenario_name, as.character(i), as.character(iterations), output_dir, as.character(folds))
      script <- create_slurm_script(
        ...
        args = args,
        num_cores = 1, time = time, memory = memory,
