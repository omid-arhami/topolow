...
run_adaptive_sampling <- function(initial_samples_file,
  ...
  if (length(missing_files) > 0) {
    stop("Failed to create temp files: ", paste(missing_files, collapse = ", "))
  }
  ...
  if (.Platform$OS.type == "windows") {
    ...
    # Wrap in tryCatch to capture and report errors
    job_results <- parallel::mclapply(seq_along(temps), function(i) {
      tryCatch({
        adaptive_MC_sampling(
          samples_file = temps[i],
          dissimilarity_matrix = dissimilarity_matrix,
        ...
        return(list(success = FALSE, job = i, error = conditionMessage(e)))
      })
    }, mc.cores = num_parallel_jobs)
    
    # Check for and report failures
    failures <- Filter(function(x) !is.null(x) && !isTRUE(x$success), job_results)
    if (length(failures) > 0 && verbose) {
      cat(sprintf("\nWarning: %d/%d parallel jobs failed.\n", 
                  length(failures), num_parallel_jobs))
      # Show first few error messages
      n_show <- min(3, length(failures))
      ...
      if (length(failures) > n_show) {
        cat(sprintf("  ... and %d more failures\n", length(failures) - n_show))
      }
    }
  }
  ...
  # Diagnostic: count how many jobs produced new samples
  jobs_with_new_samples <- sum(sapply(new_list, function(x) !is.null(x) && nrow(x) > 0))
  total_new_samples <- sum(sapply(new_list, function(x) if(!is.null(x)) nrow(x) else 0))
  
  if (verbose && total_new_samples == 0) {
    cat("\n[WARNING] No new samples were generated by any parallel job.\n")
    cat("Diagnostics:\n")
    cat(sprintf("  - Jobs that produced new samples: %d/%d\n", 
                jobs_with_new_samples, num_parallel_jobs))
    cat(sprintf("  - Temp files found: %d/%d\n", 
                sum(file.exists(temps)), length(temps)))
    # Check temp file sizes
    temp_sizes <- sapply(temps, function(f) {
      if (file.exists(f)) {
        tryCatch(nrow(read.csv(f, stringsAsFactors = FALSE)), error = function(e) 0)
      } else 0
    })
    cat(sprintf("  - Temp files with data beyond initial: %d\n", 
                sum(temp_sizes > n0)))
    cat("Possible causes:\n")
    cat("  - Filesystem sync issues (try reducing max_cores)\n")
    cat("  - Resource exhaustion (try max_cores = 20-40)\n")
    cat("  - Package loading issues in forked processes\n\n")
  }
