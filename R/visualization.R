...
#'
#' @description
#' Creates visualization of cumulative distance analysis results, either showing
#' distances from a reference point over time or pairwise distances by year difference.
#'
#' @param dist_results List output from calculate_cumulative_distances()
#' @param reference_based Logical indicating if analysis used reference point
#' @param ndim Integer number of dimensions (for file naming)
...
#' }
#' @export
plot_cumulative_distances <- function(dist_results, reference_based,
                                    ndim, scenario_name,
                                    aesthetic_config = new_aesthetic_config(),
                                    layout_config = new_layout_config()) {
  ...
  base_theme <- theme_minimal() +
    theme(
      ...
      axis.text.y = element_text(size = aesthetic_config$axis_text_size),
      axis.text.x = element_text(
        size = aesthetic_config$axis_text_size * 0.8,
        angle = 45,
        hjust = 1
      ),
  ...
  if (reference_based) {
    # Reference-based plot
    summary_data <- dist_results$summary_data
    
    plot <- ggplot(summary_data, 
                  aes(x = season_num, y = avg_euclidean_dist)) +
      geom_point(
        aes(size = fraction, fill = color),
        shape = 21,
        color = "black",
        stroke = 0.5
      ) +
      ...
      scale_x_continuous(
        breaks = seq(
          floor(min(summary_data$season_num)),
          ceiling(max(summary_data$season_num)),
          by = 1
        )
      ) +
      base_theme +
      labs(
        x = "Season",
        y = "Average antigenic distance",
        title = "Cumulative Distance Analysis (Reference-Based)"
      )
    
    filename <- sprintf(
      "%s_cumulative_dim_%d_reference.%s",
      scenario_name, ndim, layout_config$save_format
    )
    
  } else {
    # All-pairs plot
    dist_data <- dist_results$dist_data
    
    plot <- ggplot(dist_data, 
                  aes(x = as.factor(year_diff), y = euclidean_dist)) +
      geom_boxplot(
        fill = "lightblue",
        color = "grey15",
        outlier.shape = 16,
        outlier.size = aesthetic_config$point_size * 0.4
      ) +
      scale_x_discrete(
        breaks = seq(
          min(dist_data$year_diff),
          max(dist_data$year_diff),
          by = 5
        )
      ) +
      base_theme +
      labs(
        ...
        title = "Cumulative Distance Analysis (All Pairs)"
      )
    
    filename <- sprintf(
      "%s_cumulative_dim_%d_allpairs.%s",
      scenario_name, ndim, layout_config$save_format
    )
  }
  
  # Save plot if layout config specifies
  ggsave(
    filename = filename,
    plot = plot,
    width = layout_config$width,
...
#' Plot Annual Distance Analysis
#'
#' @description
#' Creates visualization of year-over-year distance analysis showing the
#' distribution of distances from previous year means.
#'
#' @param dist_results List output from calculate_annual_distances()
#' @param ndim Integer number of dimensions (for file naming)
#' @param scenario_name Character string for output file naming
#' @param outlier_threshold Numeric threshold for highlighting outliers
#' @param aesthetic_config Plot aesthetic configuration object
#' @param layout_config Plot layout configuration object 
#' @return ggplot object
#' @examples
...
#' }
#' @export
plot_annual_distances <- function(dist_results, ndim, scenario_name,
                                outlier_threshold = 4,
                                aesthetic_config = new_aesthetic_config(),
                                layout_config = new_layout_config()) {
  ...
  base_theme <- theme_minimal() +
    theme(
      ...
      axis.title = element_text(size = aesthetic_config$axis_text_size * 1.2),
      axis.text.x = element_text(
        size = aesthetic_config$axis_text_size,
        angle = 45,
        hjust = 1
      ),
      axis.text.y = element_text(size = aesthetic_config$axis_text_size),
  ...
  # Create plot
  plot <- ggplot(dist_data, aes(x = as.factor(year), y = distance)) +
    geom_boxplot(
      fill = "grey",
      color = "grey15",
      outlier.shape = 16,
    ...
    # Highlight outliers above threshold
    geom_point(
      data = subset(dist_data, distance > outlier_threshold),
      aes(x = as.factor(year), y = distance),
      color = "red",
      size = aesthetic_config$point_size * 0.4
    ) +
    labs(
      x = "Collection Year",
      y = "Distance from Previous Year Mean",
      title = sprintf(
        "Annual Distance Analysis (mean=%.2f, sd=%.2f)",
        dist_results$summary$overall_mean,
        dist_results$summary$overall_sd
      )
    ) +
    base_theme
