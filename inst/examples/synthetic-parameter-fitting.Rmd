...
opt_params <- list(
  ...
  num_parallel_jobs = 40,  # For local execution: Number of CPU cores available; For SLURM: Number of jobs to submit
...
for(i in seq_along(results$matrices)) { 
  ...
  for(matrix_type in matrix_variants) { 
    ...
    opt_result <- initial_parameter_optimization(
      ...
      num_parallel_jobs = opt_params$num_parallel_jobs,
...
amc_params <- list(
  num_parallel_jobs = 100,  # For local execution: Number of CPU cores available; For SLURM: Number of jobs to submit
  num_samples = 2000,        # Number of new samples to be added to the parameter distribution through AMC (for refinement.) If using SLURM, can set = num_parallel_jobs for better accounting.
