...
results <- generate_synthetic_datasets(
  n_dims_list = c(5, 10),
  seeds = c(903450, 13450), # 123450, seed for dim 2
  n_points = 250,
...
opt_params <- list(
  ...
  parallel_jobs = 40,  # For local execution: Number of CPU cores available; For SLURM: Number of jobs to submit
  folds = 20,         # Number of CV folds
  num_cores = 19,     # Number of cores to use. If using SLURM: 1 core per LHS sampling job. Can try parallel::detectCores() on local machines
  use_slurm = FALSE,
  cider = FALSE
)
```
...
amc_params <- list(
  parallel_jobs = 600,  # For local execution: Number of CPU cores available; For SLURM: Number of jobs to submit
  iterations = 3,          # Number of consecutive iterations per job
  max_iter = 1000,     # Maximum optimization iterations
  relative_epsilon = 1e-4
)
...
opt_params <- list(
  ...
  parallel_jobs = 40,  # For local execution: Number of CPU cores available; For SLURM: Number of jobs to submit
  folds = 20,         # Number of CV folds
  num_cores = 20,     # Number of cores to use. Can try parallel::detectCores() on local machines. It would be best if num_cores is a multiple of folds?
  use_slurm = TRUE
)
...
for(ndim in results$metadata$dimension) { 
  ...
  for(matrix_type in matrix_variants) { # 
    ...
    run_adaptive_sampling(
      ...
      folds = opt_params$folds,
      num_cores = min( opt_params$num_cores , opt_params$folds ), ###
      time = "18:00:00",
      memory = "30G",
      use_slurm = opt_params$use_slurm,
      cider = TRUE,
