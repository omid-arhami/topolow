...
```{r eval=FALSE}
# Transform samples (with previous code's path)
scenario_name = "HIV_BC_AMC202"
samples_file = "model_parameters/HIV_BC_AMC202_model_parameters.csv"
samples <- log_transform_parameters(samples_file)

# Setup adaptive sampling parameters
amc_params <- list(
  num_samples = 450, # Number of new samples to be added to the parameter distribution through AMC (for refinement.) If using SLURM, can set = num_parallel_jobs for better accounting.
  num_parallel_jobs = parallel::detectCores()-1,  # For local execution: Number of CPU cores available (Can try parallel::detectCores() ); For SLURM: Number of jobs to submit
  mapping_max_iter = 1000, # Maximum iterations per map optimization
  relative_epsilon = 1e-4,
  folds = 20, # Number of CV folds
...
```{r eval=FALSE}
# Transform samples (with previous code's path)
scenario_name = "HIV_BC_AMC203"
samples_file = "model_parameters/HIV_BC_AMC203_model_parameters.csv"
samples <- log_transform_parameters(samples_file)
