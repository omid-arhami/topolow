...
#' Prune Dissimilarity Network
#'
#' @description
#' Iteratively removes points with fewer connections than a specified minimum to create a
#' well-connected network subset.
#'
#' @param data Data frame in long format containing object and reference identifiers.
#' @param object_col Character name of the object column.
#' @param reference_col Character name of the reference column.
...
prune_dissimilarity_network <- function(data, object_col, reference_col,
                                   min_connections, iterations = 100) {
  # Check required packages
  if (!requireNamespace("dplyr", quietly = TRUE)) {
    stop("dplyr package is required for this function. Please install it.")
  }
  if (!requireNamespace("rlang", quietly = TRUE)) {
    stop("rlang package is required for this function. Please install it.")
  }
  ...
  if (!all(c(object_col, reference_col) %in% names(data))) {
    stop("Specified object and reference columns must exist in data")
  }
  ...
  n_original <- length(unique(c(data[[object_col]], data[[reference_col]])))
  ...
  while(continue_pruning && iteration < iterations) {
    ...
    object_counts <- table(pruned_data[[object_col]])
    reference_counts <- table(pruned_data[[reference_col]])

    # Identify points to remove
    low_object <- names(object_counts)[object_counts < min_connections]
    low_reference <- names(reference_counts)[reference_counts < min_connections]

    if(length(low_object) == 0 && length(low_reference) == 0) {
      continue_pruning <- FALSE
    } else {
      # Remove measurements involving points with too few connections
      pruned_data <- pruned_data %>%
        filter(!(!!sym(object_col) %in% low_object) &
                 !(!!sym(reference_col) %in% low_reference))
  ...
  n_remaining <- length(unique(c(pruned_data[[object_col]],
                                 pruned_data[[reference_col]])))
  ...
  check_connectivity <- function(data, object_col, reference_col) {
    # Create adjacency list representation
    edges <- data.frame(
      from = data[[object_col]],
      to = data[[reference_col]]
    )
  ...
  final_stats$is_connected <- check_connectivity(pruned_data, object_col, reference_col)
...
#' Prepare Data for Dissimilarity Heatmap
#'
#' @description
#' Prepares a dissimilarity matrix for heatmap visualization by handling missing values,
#' optionally clustering, and calculating relevant statistics.
#'
#' @param dissimilarity_matrix Square symmetric matrix of dissimilarities.
#' @param cluster_rows Logical; whether to hierarchically cluster rows.
#' @param cluster_cols Logical; whether to hierarchically cluster columns.
#' @return A list of data prepared for generating a heatmap:
#'   \item{matrix_data}{The dissimilarity `matrix`, potentially reordered by clustering.}
#'   \item{row_order}{An integer vector of the row indices after clustering.}
#'   \item{col_order}{An integer vector of the column indices after clustering.}
#'   \item{stats}{A list of summary statistics for the matrix.}
#' @examples
...
prepare_heatmap_data <- function(dissimilarity_matrix,
                                cluster_rows = FALSE,
                                cluster_cols = FALSE) {
  ...
  stats <- list(
    mean = mean(dissimilarity_matrix, na.rm = TRUE),
    sd = stats::sd(dissimilarity_matrix, na.rm = TRUE),
    min = min(dissimilarity_matrix, na.rm = TRUE),
    max = max(dissimilarity_matrix, na.rm = TRUE),
    completeness = sum(!is.na(dissimilarity_matrix)) / length(dissimilarity_matrix)
  )

  # Handle clustering if requested
  row_order <- seq_len(nrow(dissimilarity_matrix))
  col_order <- seq_len(ncol(dissimilarity_matrix))

  if (cluster_rows) {
    row_order <- stats::hclust(stats::dist(dissimilarity_matrix))$order
  }
  if (cluster_cols) {
    col_order <- stats::hclust(stats::dist(t(dissimilarity_matrix)))$order
  }

  # Reorder the original matrix with NAs for the final output
  matrix_data <- dissimilarity_matrix[row_order, col_order]
...
#' Plot Dissimilarity Matrix Heatmap
#'
#' @description
#' Creates a heatmap visualization of a dissimilarity matrix to show patterns and
#' structure in the measurements.
#'
#' @param heatmap_data The list output from `prepare_heatmap_data()`.
...
#'
#' @return A `ggplot` object containing the heatmap visualization.
#'
#' @examples
#' # Create a sample dissimilarity matrix
...
plot_distance_heatmap <- function(heatmap_data, output_file = NULL,
  ...
  # Prepare data for plotting by melting the matrix into a long format
  plot_data <- reshape2::melt(heatmap_data$matrix_data, varnames = c("Var1", "Var2"), value.name = "value")
  ...
  p <- ggplot2::ggplot(plot_data, ggplot2::aes(.data$Var1, .data$Var2, fill = .data$value)) +
    ggplot2::geom_tile() +
    ggplot2::scale_fill_gradient(
      low = "white",           # Fixed value instead of aesthetic_config$gradient_colors$low
      high = "darkred",        # Fixed value instead of aesthetic_config$gradient_colors$high
      na.value = "grey80",
      name = "Dissimilarity"
    ) +
    ggplot2::theme_minimal() +
    ggplot2::theme(
      plot.title = ggplot2::element_text(size = 14, hjust = 0.5),  # Fixed title size
      axis.text.x = ggplot2::element_text(angle = 90, vjust = 0.5, hjust = 1, size = 8),  # Fixed axis text size
      axis.text.y = ggplot2::element_text(size = 8),  # Fixed axis text size
      plot.margin = ggplot2::margin(10, 10, 10, 10),
      axis.title = ggplot2::element_blank()
    ) +
    ggplot2::labs(
      title = sprintf(
        "Dissimilarity Matrix (%.1f%% Complete)",
...
hiv_titers$IC50 <- clean_data(hiv_titers$IC50, k=2)
hiv_titers <- na.omit(hiv_titers)
```
...
hiv_distance_matrix <- titers_list_to_matrix(
  filtered_data,
  chnames = "Virus",
  chorder = "virusYear", 
  rnames = "Antibody",
...
results <- initial_parameter_optimization(
  dissimilarity_matrix = hiv_distance_matrix,
...
amc_params <- list(
  num_samples = 500, # Number of new samples to be added to the parameter distribution through AMC (for refinement.) 
...
diag_plots <- plot_mcmc_diagnostics(
  chain_files = chain_files,
...
# Create and display temporal mapping. -  Install umap package if not installed
