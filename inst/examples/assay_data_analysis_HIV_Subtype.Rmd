...
This vignette demonstrates how to use the topolow package to analyze HIV neutralization assay data as a real-world example of antigenic cartography. We'll perform a comprehensive analysis workflow including:

* Loading and preprocessing IC50 neutralization data for HIV subtypes "B" and "C"
* Cleaning outliers using robust median absolute deviation (MAD) methods
* Analyzing measurement coverage and connectivity patterns
* Selecting representative virus panels with good measurement coverage
* Optimizing topolow parameters using Latin Hypercube Sampling (LHS) with cross-validation
* Refining parameter estimates using Adaptive Monte Carlo (AMC) sampling
* Assessing convergence and parameter sensitivity through profile likelihood analysis
* Creating antigenic maps with cluster, temporal, and geographic visualizations
* Annotating significant viral strains and creating interactive visualizations

The vignette provides both code to perform these analyses from scratch as well as pre-computed results. If you don't want to run the computationally intensive parameter optimization sections, you can use the provided optimal parameters:

roptimal_params <- list(
    N = 2,
    k0 = 8.20368,
...
Make sure the data is connected (there are not isolated islands with no links between them.)
```{r}
...
To create reliable antigenic maps, we want to select viruses with good measurement coverage. Considering the large size of this dataset, and its median, we decide to choose top N viruses and antibodies in each year with at least 5 measurements. Let's analyze how different cutoffs (N) for minimum measurements affect our dataset:

The threshold for the minimum number of measurements per point must be greater than the dimensionality of the data. For example here, our initial estimates show that this data may need 2 to 6 dimensions, so, we define 5 measurements or less as insufficient.

```{r}
insufficient_thresh <- 5

# Function to analyze panel composition at different cutoffs
analyze_panel_sizes <- function(data, max_cutoff = 20) {
  cutoff_stats <- list()
...
opt_params <- list(
  mapping_max_iter = 500,    # Maximum optimization iterations
...
```{r eval=FALSE}
scenario_name = "HIV_BC_AMC01"
samples_file = "model_parameters/HIV_BC_AMC01_model_parameters.csv"
...
amc_params <- list(
  num_samples = 7, # Number of new samples to be added to the parameter distribution through AMC (for refinement.) If using SLURM, can set = num_parallel_jobs for better accounting.
...
positions <- read.csv("data_files/HIV_coordinates_metadata.csv")
# Non annotated
...
Topolow shows interesting separation in the antigenic space based on the location of isolates.
```{r}
...
This vignette demonstrated a complete workflow for analyzing HIV neutralization data using the topolow package, including:

1- Data preparation and quality control:

* Processing and cleaning IC50 neutralization data
* Applying robust outlier detection with the MAD method
* Analyzing and visualizing measurement coverage

2- Strategic panel selection:

* Identifying viruses and antibodies with sufficient measurement coverage
* Creating a balanced dataset with representation across years
* Generating and visualizing the distance matrix and network structure

3- Parameter optimization through multiple approaches:

* Initial exploration using Latin Hypercube Sampling with cross-validation
* Parameter refinement using Adaptive Monte Carlo sampling
* Convergence assessment across multiple independent chains
* Uncertainty quantification through profile likelihood analysis

4- Result visualization and analysis:

* Creating antigenic maps with subtype-based clustering
* Annotating significant viral isolates of interest
* Generating temporal progression visualizations
* Exploring geographic patterns in the antigenic landscape
* Creating interactive visualizations for deeper exploration


The resulting low-dimensional antigenic map effectively captures the relationships between HIV viral isolates, showing clear separation between subtypes and revealing patterns that align with both evolutionary relationships and geographic origins. These visualizations provide valuable insights into HIV antigenic structure that can inform vaccine design and epidemiological studies.
