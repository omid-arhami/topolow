...
opt_params <- list(
  ...
  num_cores = future::availableCores() - 1 # Use all available cores except one
)
    ...
    for(variant in matrix_variants) {
      ...
      while(continue_testing && test_dim <= max_test_dimension) {
        ...
        for(i in seq_len(opt_setting$n_folds)) {
          ...
          truth_matrix <- matrix_list[[i]]$truth
          input_matrix <- matrix_list[[i]]$train
          ...
          # Define the output file path for the coordinates
          coords_file <- file.path(racmacs_coords_dir, 
                                  paste0("RACMACS_dim", orig_dim, "_", variant, "_fold", i, ".csv"))

          # Run RACMACS optimization and save coordinates in one step
          ...
          racmacs_errors <- error_calculator_comparison(
            predicted_dissimilarities = p_dist_mat,
            true_dissimilarities = truth_matrix,
            input_dissimilarities = input_matrix
          )
...
plot_dimension_results <- function(results, output_dir, optimal_dims = NULL) {
  ...
  for(orig_dim in orig_dims) {
    for(variant in all_variants) {
      ...
      ggsave_white_bg(individual_file, p, width = 2.5, height = 2, units = "in")
    }
  }
  ...
  ggsave_white_bg(facet_file, facet_plot, width = 12, height = 6, units = "in")
...
for (original_dim in sim_data$metadata$dimension) {
  ...
  for (matrix_type in matrix_variants) {
    ...
    # Create CV folds 
    matrix_list <- create_cv_folds(
      dissimilarity_matrix = truth_matrix,
      ground_truth_matrix = no_noise_truth,
    ...
    for(i in seq_len(opt_params$n_folds)) {
      ...
      result <- euclidean_embedding(dissimilarity_matrix = input_matrix,
        ndim = best_params$N,
      ...
      # Define the output file path for the coordinates
      coords_file <- file.path(output_dir, "RACMACS_coords.csv")

      # Run RACMACS optimization and save coordinates
      map <- create_and_optimize_RACMACS_map(
        titer_table = titer_table,
...
After all  jobs complete, collect and analyze results:
...
# Save plot
ggsave_white_bg("Method_comparison_MAPE_plot_250points.pdf", plot = p, width = 172, height = 0.5*172, units = "mm", dpi = 300)
...
ggsave_white_bg("Method_comparison_MAE_plot_250points.pdf", plot = p, width = 172, height = 0.5*172, units = "mm", dpi = 300)
...
for(ndim in sim_data$metadata$dimension) {
  ...
  for(matrix_type in matrix_variants) { # 
    ...
    topo_result <- tryCatch({
      euclidean_embedding(dissimilarity_matrix =  distance_matrix,
        ndim = optimal_params$N,
