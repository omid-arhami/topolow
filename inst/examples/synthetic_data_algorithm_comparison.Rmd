...
all_plots <- plot_dimension_results(results = racmacs_dimensions_sim_data, output_dir, optimal_dims)
...
racmacs_2D_Completeness <- data.table(
  Dimension = numeric(),
  Mapped = numeric(),
  Total = numeric(), 
  Validation_Completeness = numeric(),
...
racmacs_TrueDim_Completeness <- data.table(
  Dimension = numeric(),
  Mapped = numeric(),
  Total = numeric(), 
  Validation_Completeness = numeric(),
...
for (ndim in sim_data$metadata$dimension) {
  ...
  for (matrix_type in matrix_variants) {
    ...
    for(i in seq_len(opt_params$n_folds)) {
      ...
      racmacs_2D_Completeness <- rbind(
        racmacs_2D_Completeness,
        data.table(
          Dimension = ndim,
      ...
      # 
      # racmacs_TrueDim_Completeness <- rbind(
      #   racmacs_TrueDim_Completeness,
      #   data.table(
      #     Dimension = ndim,
      #     Mapped = count_mapped_rows,
      #     Total = nrow(truth_matrix),
...
write.csv(racmacs_2D_Completeness, "RACMACS_mapped_objects.csv",
          row.names = FALSE)

# write.csv(racmacs_TrueDim_errors_df,"RACMACS_TrueDim_comparisondf.csv",
#           row.names = FALSE)
# 
# write.csv(racmacs_TrueDim_Completeness, "RACMACS_TrueDim_mapped_objects.csv",
#           row.names = FALSE)
```
...
racmacs_2D_Completeness <- read.csv("RACMACS_mapped_objects.csv")

#racmacs_TrueDim_Completeness <- read.csv("RACMACS_TrueDim_mapped_objects.csv")
...
racmacs_2D_Completeness_stats <- get_scenario_stats(racmacs_2D_Completeness, metric = "Mapped")
#racmacs_TrueDim_Completeness_stats <- get_scenario_stats(racmacs_TrueDim_Completeness, metric = "Mapped")
...
racmacs_2D_Completeness_stats <- add_columns(racmacs_2D_Completeness_stats, Method="MDS")
#racmacs_TrueDim_Completeness_stats <- add_columns(racmacs_TrueDim_Completeness_stats, Method="MDS")
...
```{r}
all_Completeness_results <- bind_rows(topolow_Completeness_stats, racmacs_2D_Completeness_stats)
