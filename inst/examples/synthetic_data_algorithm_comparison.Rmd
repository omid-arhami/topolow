...
plot_dimension_results <- function(results, output_dir) {
  ...
  for(orig_dim in orig_dims) {
    for(variant in variants) {
      ...
      max_mape <- max(dataset_data$MAPE)
      scaled_95_percent <- 95/max(dataset_data$Completeness)*max_mape
      
      # Create plot with dual y-axes for this dataset
      p <- ggplot(dataset_data, aes(x = Dimension)) +
        # MAPE line
        geom_line(aes(y = MAPE, color = "Validation MAPE"), size = 0.8) +
        geom_point(aes(y = MAPE, color = "Validation MAPE"), size = 1.5) +
        # Completeness line (scaled to match MAPE range for dual axis)
        geom_line(aes(y = Completeness/max(Completeness)*max_mape, color = "Completeness"), size = 0.8) +
        geom_point(aes(y = Completeness/max(Completeness)*max_mape, color = "Completeness"), size = 1.5) +
        ...
        scale_y_continuous(
          ...
          sec.axis = sec_axis(~ . * max(dataset_data$Completeness)/max_mape, name = "Completeness (%)")) +
        scale_color_manual(values = c("Validation MAPE" = "red", "Completeness" = "blue"),
                         name = "Metric") +
        labs(x = "Dimension") +
        theme_minimal() +
        theme(
  ...
  legend_data <- data.frame(
    x = 1:2,
    y = 1:2,
    Metric = c("Validation MAPE", "Completeness")
  )
  ...
  facet_plot <- ggplot(plot_data) +
    ...
    # MAPE line
    geom_line(aes(x = Dimension, y = MAPE, color = "Validation MAPE"), size = 0.6) +
    geom_point(aes(x = Dimension, y = MAPE, color = "Validation MAPE"), size = 1.2) +
    # Completeness line with secondary y-axis scaling done in annotation
    geom_line(aes(x = Dimension, y = Completeness/100*max(MAPE), color = "Completeness"), size = 0.6) +
    geom_point(aes(x = Dimension, y = Completeness/100*max(MAPE), color = "Completeness"), size = 1.2) +
    # 95% coverage reference line
    geom_hline(aes(yintercept = 0.95*max(MAPE)), linetype = "dashed", color = "blue", alpha = 0.4) +
    # Styling
    scale_color_manual(values = c("Validation MAPE" = "red", "Completeness" = "blue")) +
    labs(
      x = "Test Dimension",
