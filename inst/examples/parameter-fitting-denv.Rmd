...
amc_params <- list(
  num_samples = 3000, # Number of new samples to be added to the parameter distribution through AMC (for refinement.) If using SLURM, can set = num_parallel_jobs for better accounting.
...
```{r eval=FALSE}
# Transform samples (with previous code's path)
scenario_name = "denv_data_AMC15" #2149
samples_file = "model_parameters/denv_data_AMC15_model_parameters.csv"
samples <- log_transform_parameters(samples_file)

# Setup adaptive sampling parameters
amc_params <- list(
  num_samples = 700, # Number of new samples to be added to the parameter distribution through AMC (for refinement.) If using SLURM, can set = num_parallel_jobs for better accounting.
  num_parallel_jobs = 7,  # For local execution: Number of CPU cores available (Can try parallel::detectCores() ); For SLURM: Number of jobs to submit
  mapping_max_iter = 500, # Maximum iterations per map optimization
  relative_epsilon = 1e-4,
  folds = 20, # Number of CV folds
...
```{r}
scatterplot_fitted_vs_true(denv_distance_matrix, result$est_distances, 
                                       scenario_name = "denv", ndim = 10,
                                       save_plot = TRUE,
                                       output_dir = NULL,
