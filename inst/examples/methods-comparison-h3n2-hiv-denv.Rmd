...
denv_topolow_params <- list(
    N = 18,
    k0 = 17.63264,
    cooling_rate = 0.06207758,
    c_repulsion = 0.01812777
  )
...
denv_chains <- c(
  "model_parameters/denv_data_AMC03_model_parameters.csv",
...
dimensions_to_test <- 2:15  # Test dimensions from 1D to 10D
  ...
  for(dim in dimensions_to_test) {
    ...
    for(i in seq_len(opt_setting$n_folds)) {
      ...
      if(dataset_name %in% c("H3N2", "DENV")) {
        ...
        titer_table <- dist_to_titer_table(input_matrix, base=2, tens=1)
      }
...
result <- create_topolow_map(
  ...
  ndim = denv_topolow_params$N,
  mapping_max_iter = 200,
  k0 = denv_topolow_params$k0,
  cooling_rate = denv_topolow_params$cooling_rate,
  c_repulsion = denv_topolow_params$c_repulsion,
  relative_epsilon = 1e-10,
...
write.csv(positions, file.path(coords_dir, "topolow_DENV_coords.csv"), row.names=FALSE)
```
...
library(ape)
phylo_tree <- read.tree("data_files/phylo_groupings_rooted.nwk")
...
text_labels_topolow <- data.frame(
  x = c( 2.5, 3.6, -2.7, 2.9, -3.6, 5.3, -5, 5.3, -4, 2, -3),
...
layout_config <- new_layout_config(
  ...
  plot_margin = margin(0.05, 0.05, 0.05, 0.05, "cm"),
  save_format = "pdf",
  arrow_plot_threshold = .12
)

# Add text labels to topolow
text_labels_topolow <- data.frame(
  x = c( -2.5, -0.7, -0.4, 1.2),
  y = c( 1.2, 2, -1.5, -0.9), # Adjust y for label positioning
...
# Cluster mapping
topolow_plot_denv <- plot_cluster_mapping(positions, ndim = optimal_params$N, 
                           aesthetic_config = aesthetic_config, 
                           layout_config = layout_config, 
                           annotation_config = annotation_config,
                           draw_arrows=TRUE,
...
and compare it with the distances from Topolow coordinates in function avg_intercluster_distanceto validate the distances on the map visually.
```{r}
library(data.table)
...
# assume your data.frame 'positions' is already in memory
dt <- as.data.table(positions)
...
avg_prefix_distance(denv_long, "DENV3", "DENV4")
...
y_range <- range(clean_data(errors_df$OutSampleError, k=3.05), na.rm = TRUE)
y_padding <- abs(diff(y_range)) * 0.22  # 20% padding

# Calculate the y-position for the labels (same as existing labels)
label_y_pos <- y_range[1] - y_padding/2 - 0.1
...
p1 <- ggplot() +
  ...
  annotate(
    ...
    label = "MAE=\nSD=",
    size = stat_text_size,
    #hjust = -0.2,  # Position it outside the plot area
    vjust = 1,
    color = "black"
  ) +
  ...
  scale_y_continuous(
    limits = c(y_range[1] - y_padding, y_range[2] + y_padding/20),
    expand = c(0, 0)
  ) +
...
ggsave(file.path(plots_dir,"comparison_real_data1.pdf"),
       p1, width = 83, height = 80, units = "mm")



# # Calculate the y-axis range limits
# y_range <- range(clean_data(errors_df$OutSampleError, k=3.5), na.rm = TRUE)  # Even stricter outlier removal
# y_padding <- abs(diff(y_range)) * 0.22
# 
# # Calculate the y-position for the labels (adjusted for tighter spacing)
# label_y_pos <- y_range[1] - y_padding/2 - 0.1
# 
# p2 <- ggplot() +
#   # Add box plots with reduced width and height
#   geom_boxplot(
#     data = errors_df,
#     aes(x = Dataset, y = OutSampleError, 
#         fill = Algorithm),
#     position = position_dodge(width = 0.7),  # Narrower dodge width (from 0.8)
#     outlier.shape = NA,
#     alpha = 0.8,
#     width = 0.5,  # Even smaller width (from 0.6)
#     fatten = 0.7  # Thinner median line (from 0.8)
#   ) +
#   # Add text labels for MAE and SD
#   geom_text(
#     data = error_summary,
#     aes(x = Dataset, y = label_y_pos, 
#         label = label, group = Algorithm),
#     position = position_dodge(width = 0.7),  # Match the boxplot dodge width
#     vjust = 1,
#     size = stat_text_size,
#     color = "black"
#   ) +
...
#   # Customize appearance
#   scale_fill_manual(values = c("Topolow" = "#4dbedf", "MDS" = "#e59572")) +
#   # Very tight y-axis limits
#   scale_y_continuous(
#     limits = c(y_range[1] - y_padding, y_range[2] + y_padding/10),  # Extremely tight upper limit
#     expand = c(0, 0)
#   ) +
#   # Don't use coord_fixed - it was causing the opposite effect
#   theme_minimal(base_size = base_size) +  
#   theme(
#     axis.title = element_text(size = axis_title_size),
#     axis.text = element_text(size = axis_text_size),
...
#     #plot.margin = margin(2, 5, 2, 5),  # Smaller margins (top, right, bottom, left)
#     
#     panel.grid.major = element_line(color = "gray90"),
#     panel.grid.minor = element_blank(),
#     panel.border = element_rect(color = "black", fill = NA, linewidth = 0.5),
...
#     plot.title = element_text(size = plot_title_size, hjust = 0.5),
#     
#     # Keep legend adjustments from previous update
#     legend.position = c(0.5, 0.98),  # Move legend even higher (from 0.97)
#     legend.direction = "horizontal",
#     legend.background = element_rect(fill = "white", color = NA),
#     legend.box.background = element_rect(fill = "white", color = NA),
#     legend.margin = margin(0.1, 0.5, 0.1, 0.5),  # Tighter margins (from 0.3, 0.7)
#     legend.key.size = unit(0.7, "lines"),  # Smaller key size (from 0.8)
#     legend.title = element_text(size = legend_title_size),
#     legend.text = element_text(size = legend_text_size),
#     legend.box.margin = margin(0, 0, 0, 0),
#     
#     # Reduce space between legend and plot
...
#   )
# 
# ggsave(file.path(plots_dir,"comparison_real_data2.pdf"),
#        p2, width = 83, height = 73, units = "mm")
