...
opt_params <- list(
  ...
  parallel_jobs = 50,   # For local execution: Number of CPU cores available; For SLURM: Number of jobs to submit
  folds = 20,         # Number of CV folds
  num_cores = 6, # Number of cores to use. Try parallel::detectCores() on local machines. If using SLURM: 1 core per LHS sampling job.
  use_slurm = FALSE,
  cider = FALSE
)
```
...
results <- initial_parameter_optimization(
  ...
  scenario_name = "H3N2_2003_data_AMC11",
  ...
  parallel_jobs = opt_params$parallel_jobs,
...
scenario_name = "H3N2_2003_data_AMC11"
samples_file = "model_parameters/H3N2_2003_data_AMC11_model_parameters.csv"
samples <- log_transform_parameters(samples_file)

# Setup adaptive sampling parameters
amc_params <- list(
  parallel_jobs = 100,  # For local execution: Number of CPU cores available; For SLURM: Number of jobs to submit
  iterations = 20,        # Number of times repeating the AMC sampling
  max_iter = 1000,    # Maximum iterations per optimization
  relative_epsilon = 1e-4
)
...
opt_params <- list(
  folds = 20,         # Number of CV folds
  num_cores = 1,      # Number of cores to use. Can try parallel::detectCores() on local machines. It would be best if num_cores is a multiple of folds
  use_slurm = FALSE,
  cider = FALSE
)
...
run_adaptive_sampling(
  ...
  parallel_jobs = amc_params$parallel_jobs,
  iterations = amc_params$iterations,
...
```{r}
source("C:/Users/LibraryUser/OneDrive - University of Georgia/code/R package/topolow1/R/adaptive_sampling.R")
...
for (param in params) {
  ...
  # Plot results
  sens_plot <- plot(sensitivity, reference_error = 0.939)
