...
opt_params <- list(
  ...
  num_samples = 40, # Number of cores to use. Try parallel::detectCores() on local machines. If using SLURM: 1 core per LHS sampling job.
  use_slurm = FALSE,
  cider = FALSE
)
```
...
results <- initial_parameter_optimization(
  ...
  scenario_name = "H3N2_2003_data_AMC102",
...
write.csv(results, "model_parameters/H3N2_2003_data_AMC102_model_parameters.csv"
)
...
Aggregation is only needed if SLURM was used for initial sampling. Wait until the jobs are done.
...
scenario_name = "H3N2_2003_data_AMC101"
samples_file = "model_parameters/H3N2_2003_data_AMC101_model_parameters.csv"
samples <- log_transform_parameters(samples_file)

# Setup adaptive sampling parameters
amc_params <- list(
  num_samples = 200, # Number of new samples to be added to the parameter distribution through AMC (for refinement.) If using SLURM, can set = num_parallel_jobs for better accounting.
