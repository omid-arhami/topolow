...
```{r eval=FALSE}
scenario_name = "H3N2_2003_data_AMC202"
samples_file = "model_parameters/H3N2_2003_data_AMC202_model_parameters.csv"
...
scenario_name = "H3N2_2003_data_AMC201"
samples_file = "model_parameters/H3N2_2003_data_AMC201_model_parameters.csv"
...
```{r}
library(topolow)
positions <- read.csv("h3n2_68_03_valid_positions.csv")
...
clade_node_depths      <- 2:6
sigma_values           <- seq(0.5, 2, by = 0.5)
arrow_plot_thresholds  <- seq(0.5, 3, by = 1)
...
for (clade_depth in clade_node_depths) {
  for (arrow_thresh in arrow_plot_thresholds) {
    for (sigma_t in sigma_values) {
      for (sigma_x in sigma_values) {
        ...
        layout_config <- new_layout_config(
          ...
          sigma_t = sigma_t,
          sigma_x = sigma_x,
          arrow_plot_threshold = arrow_thresh
        )
