...
3. For phylogeny:  
   ...
   - **`dist.dna(..., model="raw")`** computes raw (Hamming) p-distances among DNA sequences.  
   - **Neighbor-Joining** recovers a topology driven purely by these distances.  
   - **Rooting** on the oldest tip approximates a temporal calibration in absence of explicit branch-length inference.  
   - **Removing edge lengths** leaves only the grouping structure.
...
# At this point, perform a multiple‐sequence alignment externally
# (e.g. MAFFT or MUSCLE), writing to
# "data_files/HA_DNA_matched_aligned.fasta"
```
