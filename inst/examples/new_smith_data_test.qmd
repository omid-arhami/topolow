...
output_dir <- file.path(getwd(), "inst", "examples")
...
```{r eval=FALSE}
scenario_name = "AMC_test"
samples_file = "/model_parameters/AMC_test_model_parameters.csv"
...
results <- initial_parameter_optimization(
  dissimilarity_matrix = h3n2_distance_matrix,
  ...
  opt_subsample = opt_params$opt_subsample,
...
write.csv(results, file.path(output_dir, samples_file), row.names=FALSE)
```
...
scenario_name = "AMC_test"
samples_file = file.path(output_dir, "model_parameters/AMC_test_model_parameters.csv")

# Setup adaptive sampling parameters
amc_params <- list(
  num_samples = 4, # Number of new samples to be added to the parameter distribution through AMC (for refinement.) 
...
# List chain files
chain_files <- c(
  "model_parameters/AMC_test_model_parameters.csv",
  "model_parameters/AMC_test2_model_parameters.csv"
)

mutual_size = 200  # Number of samples to use from end of chains
...
result <- euclidean_embedding(dissimilarity_matrix = h3n2_distance_matrix,
