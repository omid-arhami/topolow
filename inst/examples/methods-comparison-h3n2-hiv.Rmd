...
Load and prepare both input datasets of titers:
...
write.csv(hiv_titers, "data_files/hiv_assay_full.csv", row.names = FALSE)
...
This code block may take more than 7 hours. The results are provided in file 'comparison_results/racmacs_dimension_results.csv'
...
write.csv(dim_results, file.path(output_dir, "racmacs_dimension_results.csv"), row.names = FALSE)

```

## Plot MDS error vs dimension 
```{r}
...
dim_results <- read.csv('comparison_results/racmacs_dimension_results.csv')
...
save_plot(
  ...
  base_height = layout_config$height+0.5
)
```
...
This may take 15 to 60 minutes. The results needed for next steps are provided in csv file.
...
The results needed for next steps are provided in csv files, in case you have not run the previous block to create errors_df_loop.
...
```{r}
# Or read from file
fold_stats <- read.csv(file.path(output_dir, "fold_stats.csv"))

# Or Calculate fold-level statistics
...
#write.csv(error_summary, file.path(output_dir, "error_summary.csv") , row.names = FALSE)

# Or read from file
error_summary <- read.csv( file.path(output_dir, "error_summary.csv"))

# Calculate the y-axis range needed
y_range <- c(-5.12, 3.68 ) # range(clean_data(errors_df$OutSampleError, k=3.5), na.rm = TRUE)
y_padding <- abs(diff(y_range)) * 0.22  # 20% padding

# Calculate the y-position for the labels (same as existing labels)
label_y_pos <- y_range[1] - y_padding/2 - 0.2
...
p0 <- ggplot() +
  ...
  annotate(
    "text",
    x = 0.5,  # Left edge of the plot area
    y = label_y_pos,
    label = "MAE=\nSD=",
    size = stat_text_size,
...
ggsave(file.path(plots_dir,"comparison_real_data.pdf"),
       p0, width = 83, height = 80, units = "mm")
...
# Or read from the provided results in the file:
write.csv(error_dist, file.path(output_dir, "error_distribution_HIV_H3N2.csv") , row.names = FALSE)

error_dist <- read.csv(file.path(output_dir, "error_distribution_HIV_H3N2.csv"))
