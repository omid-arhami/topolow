...
create_and_optimize_RACMACS_map <- function(titer_table, 
  ...
  if (!requireNamespace("Racmacs", quietly = TRUE)) {
    stop("Package 'Racmacs' is required for this function. Please install it.")
  }
  library(Racmacs)
...
# New
#' Calculate Statistical Significance of Procrustes Difference
#'
#' @description
#' Performs a Procrustes permutation test to calculate the statistical significance 
#' of the difference between two point configurations (maps). This helps 
#' determine if the similarity between the two configurations is greater than expected 
#' by chance.
#'
#' @param coords1 A numeric matrix of coordinates from the first configuration, where rows are points and columns are dimensions.
#' @param coords2 A numeric matrix of coordinates from the second configuration, with dimensions matching `coords1`.
#' 
#' @return A single numeric p-value from the Procrustes permutation test. A small 
#'   p-value suggests the configurations are more similar than expected by chance.
#'   
#' @examples
#' # Create two sample 3D coordinate matrices
#' coords1 <- matrix(rnorm(30), nrow = 10, ncol = 3)
#' coords2 <- coords1 + rnorm(30, sd = 1.5) # Add significant noise
#' 
#' # Calculate the significance of the difference
#' p_val <- calculate_procrustes_significance(coords1, coords2)
#' print(p_val)
#' 
calculate_procrustes_significance <- function(coords1, coords2) {
  if (!requireNamespace("vegan", quietly = TRUE)) {
    stop("Package 'vegan' is required for this function. Please install it.")
  }
  # --- Input Validation ---
  if (!is.matrix(coords1) || !is.numeric(coords1) || 
      !is.matrix(coords2) || !is.numeric(coords2)) {
    stop("Inputs must be numeric matrices.")
  }
  if (!identical(dim(coords1), dim(coords2))) {
    stop("Coordinate matrices must have the same dimensions.")
  }
  if (any(!is.finite(coords1)) || any(!is.finite(coords2))) {
    stop("Coordinate matrices must contain finite values.")
  }

  # --- Procrustes Permutation Test ---
  # The vegan::protest function performs the permutation test
  procrustes_result <- vegan::protest(coords1, coords2)
...
#' Calculate Procrustes Difference Between Two Configurations
#'
#' @description 
#' Computes the quantitative difference between two point configurations (maps) 
#' using Procrustes analysis. The function calculates the sum of squared 
#' differences between the two sets of coordinates after optimal rotation, 
#' translation, and uniform scaling.
#'
#' @param coords1 A numeric matrix of coordinates from the first configuration, where rows are points and columns are dimensions.
#' @param coords2 A numeric matrix of coordinates from the second configuration, with dimensions matching `coords1`.
#' 
#' @return A single numeric value representing the sum of squared differences after the Procrustes transformation.
#' 
#' @examples
#' # Create two sample 3D coordinate matrices
#' coords1 <- matrix(rnorm(30), nrow = 10, ncol = 3)
#' # Create a second set of coordinates that is a slightly rotated version of the first
#' angle <- pi / 8
#' rotation_matrix <- matrix(c(cos(angle), -sin(angle), 0, sin(angle), cos(angle), 0, 0, 0, 1), 3, 3)
#' coords2 <- coords1 %*% rotation_matrix + rnorm(30, sd = 0.1) # Add some noise
#' 
#' # Calculate the Procrustes sum of squares difference
#' diff <- calculate_procrustes_difference(coords1, coords2)
#' print(diff)
#' 
calculate_procrustes_difference <- function(coords1, coords2) {
  if (!requireNamespace("vegan", quietly = TRUE)) {
    stop("Package 'vegan' is required for this function. Please install it.")
  }
  # --- Input Validation ---
  ...
  # --- Procrustes Analysis ---
  # Perform Procrustes analysis to find the optimal alignment and difference
  procrustes_result <- vegan::procrustes(coords1, coords2, scale = TRUE)
...
hiv_matrix <- titers_list_to_matrix(filtered_data,
                            chnames = "Virus",
                            rnames = "Antibody", 
...
DENV_titers <- read.csv("data_files/DENV_titers.csv", stringsAsFactors = FALSE)
...
generate_topolow_maps <- function(distance_matrix, params, valid_points, n_maps = 15) {
  ...
  for(i in 1:n_maps) {
    res <- euclidean_embedding(dissimilarity_matrix =  distance_matrix,
      ndim = params$N, 
